let logger = function () {
    ...
    visible.DATA_DEMO_END_STATE = "DEMO_END_STATE"; // state of dataset + what was accepted - Note: assumes that demo
    ...
    visible.itemString = function (id) {
        let item = model.items[id];
        let list = item.listID === model.list1.id ? model.list1.name :
            model.list2.name;
    ...
    function entryID() {
        let entryNumber = utils.getStorageItem(ENTRY_NUMBER);
