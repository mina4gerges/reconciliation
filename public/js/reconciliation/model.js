let model = function () {
    ...
    visible.viewData = function (sort, filter) {
        ...
        let relevantIds = visible.list1.source.concat(visible.list2.source);

        // get or hide shadows
        if (visible.multigroup && visible.groupBy) {
            for (let shadowID in visible.shadows) {
                let shadow = visible.shadows[shadowID];
        ...
        for (let i in relevantIds) {
            ...
            let item = visible.items[id];

            let trueId = item.isShadow ? parseInt(visible.shadowsToItems[id]) : id;

            // populate "groups"

            let itemGroup = visible.DEFAULT_GROUP;
    ...
    function populateShadows() {
        ...
        for (let id in visible.items) {
            ...
            for (let attributeName in visible.attributes) {
                let attribute = visible.attributes[attributeName];
                    ...
                    if (attributeName in item.attributes) {
                        ...
                        if (values.length > 1) {
                            ...
                            for (let i = 1; i < values.length; i++) {
                                ...
                                let shadow = new ListItem(shadowID, item.listID, item.getNames(), item.attributes);
    ...
    function detectDrugClasses() {
        ...
        for (let id in visible.items) {
            ...
            let drugClasses = item["attributes"][visible.ATTR_DRUG_CLASS];
    ...
    function detectDiagnoses() {
        ...
        for (let id in visible.items) {
            ...
            let diagnoses = item["attributes"][visible.ATTR_DIAGNOSES];
    ...
    function groupThenSort(a, b) {
        ...
        let groupOrder = visible.groupBy ? attributeSort(a, b, visible.groupBy, visible.attributes[visible.groupBy].rank) : 0;

        // if no groupOrder, try sortOrder
        //  if no sortOrder, just sort by ids
        if (groupOrder === 0) {
            let sortOrder = visible.sortBy ? attributeSort(a, b, visible.sortBy) : visible.items[a].id - visible.items[b].id;
    ...
    function attributeSort(a, b, attribute, rank) {
        let itemA = visible.items[a];
        let itemB = visible.items[b];
